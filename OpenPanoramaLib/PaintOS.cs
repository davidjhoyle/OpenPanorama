    ...
    public class PaintOS
    {
        ...
        public void DrawContours(PaintImage pi, Graphics profGraph, XDocument xdoc, double lat, double lon, double hght, countryEnum eCountry)
        {
            ...
            for (int hi = 0; hi < heights.Length; hi++)
            {
                ...
                for (int i = 0; i < xs[hi].Length; i += 1)
                {
                    int nearest_X = 0;
                    int nearest_y = 0;
                    int nearest_z = 0;
                    ...
                    {
                        double min_dist = 0;
                        bool nearest = GetNearestLowerPoint( heights, xs, ys, heights[hi], xs[hi][i], ys[hi][i], ref nearest_X, ref nearest_y, ref nearest_z, ref min_dist, chint);
                        if (nearest)
                        {
                            ...
                            Gradient = min_dist / (heights[hi] - nearest_z);
        ...
        public void DrawSpotHeights(PaintImage pi, Graphics profGraph, XDocument xdoc, double lat, double lon, double hght, countryEnum eCountry)
        {
            ...
            foreach (XElement spot in allSpotHeights)
            {
                ...
                Angle = Angle * 180 / Math.PI * pi.rjParams.pixels;
                    ...
                    pi.ZBuffer[(int)Angle][(int)Elevation] > distance)
                {
                    ...
                    {
                        string val = h + " " + ll.lat.ToString("0.00000") + ", " + ll.lon.ToString("0.00000");
